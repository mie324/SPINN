...
import os
import numpy as np
from sensegram_models_location import *
import datetime
import pandas as pd
...
def load_data():
    global bs,eval_bs
    train_df = pd.read_pickle('train1location_sensegram.pkl')
    val_df = pd.read_pickle('validation1location_sensegram.pkl')
    test_df = pd.read_pickle('test1location_sensegram.pkl')
    train_dataset = Sensegram_Data(train_df[['score']].values,train_df['location'].values)
    val_dataset = Sensegram_Data(val_df[['score']].values,val_df['location'].values)
    test_dataset = Sensegram_Data(test_df[['score']].values,test_df['location'].values)
