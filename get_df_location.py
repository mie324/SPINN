...
#location_array = pun_df['location'].values
#np.save('sensegram_location_arr.npy',location_array)

sensegram_df = pd.DataFrame()
#stacked_score_tensor= torch.ones(2)
...
for i in range(1255):
    sentence = pun_df['text'][i]
    location = pun_df['location'][i]
    ...
    sensegram_df = sensegram_df.append({'score':sentence_score_array,'location':location},ignore_index=True)
...
sensegram_df.to_csv('sensegram_scores.tsv', sep='\t',index=False)
sensegram_df.to_pickle('sensegram_scores.pkl')
